...
const convertCase = (str: string, toCase = 'camel') => {
  if (!str) return '';
  ...
  const transform = ['camel', 'pascal'].includes(toCase)
    ? (x: any) => x.slice(0, 1).toUpperCase() + x.slice(1).toLowerCase()
    : ['snake', 'kebab'].includes(toCase)
    ? (x: any) => x.toLowerCase()
    : toCase === 'title'
    ? (x: any) => x.slice(0, 1).toUpperCase() + x.slice(1)
    : (x: any) => x;

  const finalTransform =
    toCase === 'camel'
      ? (x: any) => x.slice(0, 1).toLowerCase() + x.slice(1)
      : toCase === 'sentence'
      ? (x: any) => x.slice(0, 1).toUpperCase() + x.slice(1)
      : (x: any) => x;
